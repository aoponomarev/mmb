...
// - tooltipIcon (String) — всплывающая подсказка for иконки слева (по умолчанию нативная через title, можно переключить на Bootstrap через tooltipIconBootstrap)
// - tooltipText (String) — всплывающая подсказка for текстовой области (по умолчанию нативная через title, можно переключить на Bootstrap через tooltipTextBootstrap)
// - tooltipSuffix (String) — всплывающая подсказка for суффикса (приоритет над suffix.tooltip, по умолчанию нативная через title, можно переключить на Bootstrap через tooltipSuffixBootstrap)
// - tooltipIconBootstrap (Boolean, default: false) — использовать Bootstrap tooltip for иконки вместо нативной подсказки
// - tooltipTextBootstrap (Boolean, default: false) — использовать Bootstrap tooltip for текста вместо нативной подсказки
// - tooltipSuffixBootstrap (Boolean, default: false) — использовать Bootstrap tooltip for суффикса вместо нативной подсказки
...
// - classesAdd (Object, default: {}) — классы for добавления на различные элементы компонента. Структура: { root: 'классы', icon: 'классы', subtitle: 'классы', suffix: 'классы' }
// - classesRemove (Object, default: {}) — классы for удаления с различных элементов компонента. Структура: { root: 'классы', icon: 'классы', subtitle: 'классы', suffix: 'классы' }
//
...
// - Компонент использует класс dropdown-item Bootstrap for базовой стилизации
...
// - Если соответствующий prop = true, using Bootstrap tooltip с инициализацией через window.bootstrap.Tooltip
// - Bootstrap tooltips уничтожаются в beforeUnmount() for предотвращения утечек памяти
// - Раздельные подсказки for иконки (tooltipIcon), текста (tooltipText) и суффикса (tooltipSuffix или suffix.tooltip)
// Обработка событий:
...
// - Используется @mouseup вместо @click for закрытия dropdown при отпускании кнопки мыши
...
window.cmpDropdownMenuItem = {
    ...
    computed: {
        // Подсказка for суффикса (приоритет у tooltipSuffix)
        suffixTooltip() {
            return this.tooltipSuffix || (this.suffix && this.suffix.tooltip) || null;
        },

        // CSS классы for корневого элемента
        itemClasses() {
        ...
        // CSS классы for контейнера текстовой области
        ...
        // CSS классы for области заголовка
        titleClasses() {
    ...
    methods: {
        ...
        initTouchTooltips() {
            ...
            // Отслеживание изменений title for обновления Bootstrap tooltips
                            ...
                            if (tooltipEntry && tooltipEntry.tooltip) {
                                ...
                                if (newTitle) {
                                    ...
                                    try {
                                        ...
                                        // Также обновляем data-original-title for согласованности
                                        element.setAttribute('data-original-title', newTitle);
                                    } catch (error) {
    ...
    mounted() {
        // Инициализация Bootstrap tooltips только for элементов, где соответствующий prop = true
        this.$nextTick(() => {
