...
- **#for-plan-flexibility-in-doc** Every plan in `docs/plans/` MUST include a "Гибкость плана" (or "Plan Flexibility") section at the very beginning — immediately after Context and before Цель/Scope. This paragraph states that the plan is a living document and agents may amend it during execution. Makes the iterative improvement concept visible in-context; reference id:sk-8f9e0d Rule 7.
...
1.  **Phase 1: Planning (`docs/plans/`)**
    ...
    - Language: Russian.
    - **Plan Flexibility paragraph (#for-plan-flexibility-in-doc)**: Every new plan MUST include a "Гибкость плана" (or "Plan Flexibility") section at the very beginning, immediately after the Context block and before Цель/Scope. The paragraph states that the plan is a living document and agents may amend it during execution per id:sk-8f9e0d (process-plan-execution). Mandatory for all plans.
    ...
    - **Execution protocol (`#for-plan-execution-protocol`)**: When executing any plan, follow id:sk-8f9e0d (process-plan-execution): verify each step (console checks), update AIS after each architecturally significant step (#for-continuous-ais-during-plan), add skills/causalities/contracts as discovered, fix bugs along the way. No step without verification; no deferred documentation.
...
3.  **Phase 3: Distillation (`docs/ais/` and `is/skills/`)**
    - Upon user request, an AI agent performs "distillation" of the completed plan. AIS has been updated continuously during execution (#for-continuous-ais-during-plan); distillation is the final cleanup pass — merge remaining narrative, polish prose, extract skills, then delete plan.
