...
> **Context**: At the anchor level, agents can optionally specify whether a causality expresses a **constraint** (из-за чего — because of what) or a **goal** (для чего — for what). Same hash, different semantic nuance per use site.
...
1. **Optional suffix** — After the hash, add `constraint` or `goal` when the distinction matters. Omission is valid; existing anchors need not be updated.
...
4. **Format** — `@causality #for-<hash> constraint` or `@causality #for-<hash> goal`. Same for `@skill-anchor id:sk-8991cd #for-<hash> constraint`.

### Examples

```javascript
// constraint — из-за лимитов API
// @causality #for-rate-limiting constraint

// goal — для трассируемости
