...
> **SSOT**: Template `#JS-EsMQyEpA (file-header-template.js)`. AIS: `id:ais-f7e2a1 (docs/ais/ais-file-header-standard.md)`. Contract: `#JS-Am2QGp6w (file-header-contract.js)`. Plan: `id:plan-f7e2a1 (docs/plans/file-header-rollout.md)`.
...
5. **New files** — Copy header from `#JS-EsMQyEpA`; run `npm run file-headers:fix` to update registry and get correct file id.
...
- **Contract**: `#JS-Am2QGp6w` — FILE_ID_PATTERN, REQUIRED_HEADER_FIELDS, ALLOWED_HEADER_TAGS, getExpectedFileId, getFileIdFromHeader, validateHeaderFull, extractHeaderComment.
