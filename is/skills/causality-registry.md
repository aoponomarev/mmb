...
| `#for-mcp-data-contour` | MCP data lives in two domains: SSOT (id-registry, code-file-registry, causality-registry) in JSON/MD — read directly; runtime (events, dependency_graph, raw_causalities) in data/mcp.sqlite. No cache of registries in SQLite avoids sync drift. AIS id:ais-8d3c2a. |
