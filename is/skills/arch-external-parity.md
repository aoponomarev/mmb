...
> **Context**: Defines the strategy for maintaining compatibility between Legacy PF and PF external infrastructure contracts during the migration period.

## Reasoning

- **#for-functional-parity** PF must produce equivalent external behavior to Legacy without copying its flawed internal implementations.
- **#for-single-writer-guard** PF and Legacy PF share Cloudflare/Yandex stores, so a strict `DATA_PLANE_ACTIVE_APP` variable guarantees only one environment can mutate data at a time.
...
- **#for-env-contract-sync** `.env.example` serves as the exact intersection contract for both Legacy PF and PF.
