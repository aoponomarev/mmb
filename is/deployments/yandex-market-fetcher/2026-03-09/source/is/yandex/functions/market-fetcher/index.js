...
function resolveOrderFromEvent(event) {
    const isValidOrder = (value) => value === 'market_cap' || value === 'volume';
    ...
    try {
        ...
        } else if (bodyRaw && typeof bodyRaw === 'object' && isValidOrder(bodyRaw.order)) {
            return bodyRaw.order;
        }
    } catch (e) {
        // Ignore malformed manual payload and fall back to schedule-based routing.
    }

    const directOrder = event?.order;
    if (isValidOrder(directOrder)) {
        return directOrder;
    }

    // @causality #for-trigger-minute-routing
    // Two timer triggers reuse one function artifact, so minute-based fallback
    // keeps deterministic cap/volume routing when no explicit manual order exists.
    const minute = new Date().getMinutes();
    return minute < 15 ? 'market_cap' : 'volume';
}
...
module.exports.handler = async function (event, context) {
    ...
    try {
        ...
        const order = resolveOrderFromEvent(event);
