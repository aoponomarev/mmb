...
function invokeCoingeckoFetcher(payload) {
    return new Promise((resolve, reject) => {
        ...
        const bodyPayload = JSON.stringify(payload || { order: 'market_cap' });
    ...
    const client = new PostgresAdapter(dbConfig, { ClientClass: Client });
        ...
        if (path.endsWith('/api/coins/market-cache') && method === 'GET') {
            ...
            if (ids && ids.length > 0) {
                ...
                // Include all cloud coins, not only ranked history rows.
                // Registry modes may have NULL sort fields but valid market data.
                query = `SELECT * FROM coin_market_cache
                         ORDER BY total_volume DESC NULLS LAST, sort_volume ASC NULLS LAST
                         LIMIT $1`;
                params = [limit];
            } else {
                query = `SELECT * FROM coin_market_cache
                         ORDER BY market_cap DESC NULLS LAST, sort_market_cap ASC NULLS LAST
                         LIMIT $1`;
                params = [limit];
            }
        ...
        if (path.endsWith('/api/coins/registry') && method === 'GET') {
            ...
            const wrappedRes = await client.query(
                `SELECT coin_id FROM coin_registry WHERE type = 'wrapped' ORDER BY coin_id`
            );
            const lstRes = await client.query(
                `SELECT coin_id FROM coin_registry WHERE type = 'lst' ORDER BY coin_id`
            );
            const stableRes = await client.query(
                `SELECT peg, array_agg(coin_id ORDER BY coin_id) as ids
                 FROM coin_registry WHERE type = 'stable' GROUP BY peg`
            );
            const commodityRes = await client.query(
                `SELECT peg, array_agg(coin_id ORDER BY coin_id) as ids
                 FROM coin_registry WHERE type = 'commodity' GROUP BY peg`
            );

            const fiat = {};
            (stableRes.rows || []).forEach(r => {
                const peg = (r.peg || 'usd').toLowerCase();
                fiat[peg] = Array.isArray(r.ids) ? r.ids : [];
            });
            const commodity = {};
            (commodityRes.rows || []).forEach(r => {
                const peg = (r.peg || 'gold').toLowerCase();
                commodity[peg] = Array.isArray(r.ids) ? r.ids : [];
            });

            const maxUpdated = await client.query(
                `SELECT COALESCE(MAX(updated_at), to_timestamp(0)) as ts FROM coin_registry`
            );
            const updatedAt = maxUpdated.rows[0]?.ts
                ? Math.floor(new Date(maxUpdated.rows[0].ts).getTime())
                : 0;

            const body = {
                stable: { fiat, commodity },
                wrapped: (wrappedRes.rows || []).map(r => r.coin_id),
                lst: (lstRes.rows || []).map(r => r.coin_id),
                updatedAt
            };
        ...
        if (path.endsWith('/api/coins/cycles') && method === 'GET') {
            try {
                ...
                const registryRes = await client.query(`
                    WITH registry_groups AS (
                        SELECT
                            CASE
                                WHEN type = 'stable' THEN 'registry_fiat'
                                ELSE 'registry_wlc'
                            END AS sort_type,
                            coin_id,
                            updated_at
                    ...
                    aggregated AS (
                        SELECT
                            sort_type,
                            COUNT(DISTINCT coin_id)::TEXT AS coin_count,
                            MIN(updated_at) AS started_at,
                            MAX(updated_at) AS finished_at
                        FROM registry_groups
                        GROUP BY sort_type
                    )
                    SELECT
                        TO_CHAR(finished_at, 'YYYYMMDDHH24MISS') || '-' || sort_type AS cycle_id,
                        coin_count AS row_count,
                        coin_count AS coin_count,
                        sort_type,
                        started_at,
                        finished_at
                    FROM aggregated
                ...
                const mergedCycles = [
                    ...
                    .sort((a, b) => {
                        const aTs = new Date(a.finished_at || a.started_at || 0).getTime();
                        const bTs = new Date(b.finished_at || b.started_at || 0).getTime();
                        return bTs - aTs;
                    })
                    .slice(0, 8);
                ...
                return {
                    statusCode: 200,
                    headers: CORS_HEADERS,
                    body: JSON.stringify({ cycles: mergedCycles })
                };
            } catch (histErr) {
                return {
                    statusCode: 200,
        ...
        if (path.endsWith('/api/coins/market-cache/trigger') && method === 'POST') {
            ...
            try {
                const result = await invokeCoingeckoFetcher(payload);
                return {
