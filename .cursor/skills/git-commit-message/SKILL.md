...
Use the module, folder, or feature name: `api`, `ui`, `skills`, `mcp`, `infra`, `plans`, etc. Omit if the change is truly cross-cutting.
...
- `[SSOT: <path>]` — if the commit modifies a Single Source of Truth file (e.g., `paths.js`, `.env.example`).
...
**Small fix:**
```
fix(api): handle null response from CoinGecko provider

The provider occasionally returns null when no coins match the query.
...
```
feat(infra): migrate cache paths to SSOT

Moved all temporary file writing to use the global paths.js contract
to prevent scattered hardcoded paths in the codebase.

[SSOT: is/contracts/paths/paths.js]
