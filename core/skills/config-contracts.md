...
Lib loading is governed by process-lib-governance. SSOT для library versions — см. core/ (module-loader, libs config). Directory structure: vendor libs only — vue, chartjs, etc. (libs/<name>/<version>/). Coin/data assets live in a/ root. Load priority: GitHub Pages (primary for Web) → CDN (backup) → Local `file://` (primary for Dev/Offline). Usage: `await window.libLoader.load('vue', '3.4.0')`. Version lock in `LIB_SOURCES` is mandatory; the browser cannot write to disk — user must run `download-libs.sh` for offline support.
