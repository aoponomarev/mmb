...
В MBB все мета-скилы лежали в `process/` вперемешку с обычными процессными скилами.
Это создавало навигационный шум. В MMB `meta/` — отдельная категория с явной семантикой.
...
Выявлено 30 мета-скилов в MBB (ревизия 2026-02-20):
...
В MBB `.cursorrules` уже был деградирован до "personality controller" (краткость, стиль),
а реальные правила жили в `.cursor/rules/*.mdc`. В MMB идём дальше — `.cursorrules`
не создаётся вообще.
