...
Эти данные создаются скриптами и MCP; в git не коммитятся (`data/*.sqlite`, `data/*.sqlite-wal`, `data/*.sqlite-shm` в `.gitignore`).
...
4. dependency_graph заполняется #JS-eG4BUXaS (validate-causality-invariant.js); MCP только читает.
