...
- **MCP Ecosystem:** Локальный Node.js сервер (`is/mcp/index.js`), который проксирует инструменты (Tools) для ИИ-агентов. Все действия агентов логируются в локальную базу `data/mcp.sqlite`.
