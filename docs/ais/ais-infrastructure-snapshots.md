...
Примечание по rollout-gap (`#for-ais-rollout-gap-marking`): целевой lifecycle для этого `AIS` требует archive только после `deploy + minimal verification`. Текущие deploy wrappers для active targets всё ещё вызывают `archive-deployment-snapshot.js` сразу после успешного deploy. Это временный переходный зазор: target state остаётся прежним, а отклоняющиеся ветки в коде обязаны быть помечены inline-комментарием до доведения wrappers до полного verify-before-archive поведения.
...
- Текущее состояние rollout: verify-before-archive и auto-cleanup-on-post-check-failure ещё не полностью встроены в deploy wrappers; до закрытия этого зазора архивирование остаётся автоматическим, но не полностью lifecycle-complete относительно target state.
