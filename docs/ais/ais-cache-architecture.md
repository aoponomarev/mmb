...
- `coins-metadata-v2` — реестр stable/wrapped/lst/commodity. **Primary:** Yandex API `GET /api/coins/registry` (PostgreSQL `coin_registry`); **fallback:** GitHub CDN `a/data/coins.json`. Cloud cron (market-fetcher) заполняет `coin_registry` по CoinGecko категориям: `wrapped-tokens`, `liquid-staking`, `tokenized-gold/silver/commodities`, per-currency stablecoins (`eur/gbp/jpy/chf/cny/try/rub-stablecoin` + `stablecoins` catch-all). Commodity peg: gold, silver, platinum, palladium, uranium, copper, oil, other. Stable peg: usd, eur, gbp, jpy, chf, cny, try, rub. Runtime loader (`coins-metadata-loader.js`) пробует Yandex, при ошибке — GitHub; разворачивает peg-группы в единый список для `coinsConfig`, эмитит `coins-metadata-updated`. В `#JS-W23K9iSC (coin-set-load-modal-body.js)` registry-наборы в модалке отображаются одной компактной строкой из трех горизонтальных секций (stable / wrapped / lst) с иконками и счетчиками (`💲🪙⛏️`, `🔁`, `🔥`) и выравниванием left/center/right; данные секций строятся как пересечение registry ids с cloud `market-cache`. В `app/app-ui-root.js` dropdown-выбор использует `coinsConfig.getCoinType()`, `getStablecoinPegLabel()` и `getStablecoinBaseCurrency()`; non-USD stable-разделение: `Другие фиаты` отдельно от `Металлы и сырьё`, а row badge для commodity-backed stablecoins отображается как `⛏️` вместо общего `🪙`.
...
4. **coins.json flow (#for-curated-wrapped-lst-preservation):** Облачный крон (market-fetcher) заполняет `coin_market_cache` и `coin_registry` (режимы registry_wlc, registry_fiat). Файл `a/data/coins.json` обновляется только через `coins-metadata-generator` (браузер) при нажатии «Update metadata». Генератор: stablecoins — из market-cache + peg-детекции; wrapped и lst — **только** сохраняет curated из существующего metadata (никаких эвристик). Классификация в UI — строго registry-only (`coinsConfig.getCoinType` без fallback на эвристики). Runtime loader: primary = Yandex `/api/coins/registry`, fallback = GitHub CDN `coins.json`.
   Hardening: registry режимы делают safety replace (no `DELETE` before successful fetch; transaction + threshold), чтобы 429/partial CoinGecko failure не обнулял registry.
