...
- **Язык** — все комментарии в коде только на английском (SSOT: process-language-policy); гейт #JS-GcQ9UGZD (is/scripts/architecture/validate-code-comments-english.js) уже обеспечивает это.
...
- Язык шапки — только английский (проверяется существующим гейтом #JS-GcQ9UGZD (is/scripts/architecture/validate-code-comments-english.js)).
...
|------|----------|------------|
| #JS-EsMQyEpA | shared/templates/file-header-template.js | SSOT шаблона шапки; копировать при создании/обновлении файла. |
...
| #JS-1E2YRywQ | is/scripts/architecture/assign-file-ids.js | Построение и запись реестра: обход SCAN_DIRS, вычисление file id (djb2+Base58 от пути); файлы не изменяет. Вызов: `npm run file-headers:assign`. |
...
- **Preflight**: после гейта «code comments English» выполняется #JS-zh26RZvs (is/scripts/architecture/validate-file-headers.js) (без --fix); при падении preflight не проходит.
