...
- **Файловый идентификатор** — каждый файл помечается хэшем вида `#<EXT>-<short_hash>`, где EXT — тип (JS, TS, CSS, HTML и т.д.), hash — детерминированный короткий хэш от канонического относительного пути (алгоритм: djb2 от пути с нормализованными слэшами, затем Base58, длина 8 символов). Документация и скиллы ссылаются на файл по этому id. Для первого важного упоминания используется смешанный режим: `#JS-... (basename.js)` при уникальном basename или `#JS-... (repo/path.js)` при коллизии basename; повторные упоминания в том же файле сокращаются до bare hash. При переименовании или перемещении файла путь меняется — id пересчитывается; после запуска #JS-1E2YRywQ (assign-file-ids.js) реестр и шапку в файле нужно обновить.
...
- **Язык** — все комментарии в коде только на английском (SSOT: process-language-policy); гейт #JS-GcQ9UGZD (validate-code-comments-english.js) уже обеспечивает это.
...
|------------------|------|--------------------|
| Ссылки на код по пути в скиллах | STALE_PATH | Заменить на #JS-xxx; для первого важного упоминания оставить basename/full path по mixed mode |
