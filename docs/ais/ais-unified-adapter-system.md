...
4. **Registry policies are SSOT** — order, allowlist и health thresholds живут в `core/config/adapter-registry-config.js`, а не рассеиваются по отдельным модулям. Для `Coin data` это включает runtime-aware named policies (`pg-primary-only`, `pg-primary-then-selected-external`, `selected-external-only`); фасад работает через явный `policyKey`, а не через ad-hoc boolean flags.
