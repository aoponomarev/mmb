...
# AIS: Контур данных MCP (MCP Data Contour)

## Идентификация и цель

- id: `ais-8d3c2a` — спецификация размещения и чтения данных MCP-экосистемой.
- Цель: единый контур без дублирования — SSOT в JSON/MD, runtime-данные в SQLite; MCP читает напрямую, без кэша между слоями.
...
| `is/skills/causality-registry.md` | хэши #for-X / #not-Y и формулировки |
...
| `dependency_graph` | source_hash → target_file (из validate-causality-invariant) |
...
3. **Инвариант**: Скрипты (preflight, validate-causality-invariant) пишут в JSON/MD и в DB по своим доменам. MCP только читает.
...
3. dependency_graph заполняется validate-causality-invariant; MCP только читает.
4. Переименование telemetry.sqlite → mcp.sqlite зафиксировано в коде и спеках.

## Ссылки

- `is/mcp/db.js` (#JS-YD283xUP)
- `is/mcp/resources.js` (#JS-HU3hEyDe)
- id:sk-3225b2 (arch-mcp-ecosystem)
- id:backlog-69de5d (mcp-ecosystem-roadmap)
