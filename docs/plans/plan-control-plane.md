...
- [ ] Инвентаризировать текущие control-plane сценарии Legacy App.
- [ ] Определить минимальный target-control-plane в Target App (Control Plane v1).
- [ ] Описать bootstrap/health/restart контракты.
- [ ] Зафиксировать базовую карту зависимостей и границы с backend/integrations.
- [ ] Зафиксировать динамическую fail-policy для runtime parity gate (P0/P1/P2).
- [ ] Добавить runbook `docs/runbooks/control-plane-v1.md`.
- [ ] Построить adapter-map Legacy App→Target App: `docs/runbooks/control-plane-adapter-map.md`.
- [ ] Создать bootstrap скрипт: `scripts/infrastructure/bootstrap-mmb-infra.ps1`.
- [x] Создать health-check скрипт: `scripts/infrastructure/health-check.js` (`npm run cp:health`).
- [ ] Верифицировать полный operational command set консольно (все gates зелёные).
