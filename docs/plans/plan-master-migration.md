...
### Этап 2: Базовая инфраструктура и Control Plane — *Требует внимания (Следующий приоритет)*
- [x] Очистка истории Git от случайно попавших секретов, настройка переменных окружения (Secret Resilience MVP).
- [ ] Перенос утилит Control Plane (например, `scripts/infrastructure/health-check.js`).
- [ ] Внедрение и верификация single-writer guard (`DATA_PLANE_ACTIVE_APP`) для защиты общих данных в облаке.
